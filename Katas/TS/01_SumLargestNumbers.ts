...
const sumLargestNumbers = (arr: number[]): number => {
	const firstNum: number = Math.max(...arr);
	let secondNum: number = 0;

	arr.forEach((elem) => {
		if (elem > secondNum && elem !== firstNum) secondNum = elem;
	});
...
// alternative logic
const sumLargestNumbersAlt = (arr: number[]): number => {
	const arrCopy: number[] = [...arr];

	const firstNum: number = Math.max(...arrCopy);
	arrCopy.splice(arrCopy.indexOf(firstNum), 1);
	const secondNum: number = Math.max(...arrCopy);
